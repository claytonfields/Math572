...
LSE = (sp.log(N)-sp.log((K*N_0)/(N_0 + (K-N_0)*sp.exp(-r*t))))**2
dLdr = sp.diff(LSE,r)
dldK = sp.diff(LSE,K)


def dLdr(N,N_0,K,r,t):
    return (-2*t*(K - 2)*(log(N) - log(2*K/((K - 2)*exp(-r*t) + 2)))*exp(-r*t)/((K - 2)*exp(-r*t) + 2))
    
def dLdk(N,N_0,K,r,t):
    return -((K - 2)*exp(-r*t) + 2)*(-2*K*exp(-r*t)/((K - 2)*exp(-r*t) + 2)**2 + 2/((K - 2)*exp(-r*t) + 2))*(log(N) - log(2*K/((K - 2)*exp(-r*t) + 2)))/K
